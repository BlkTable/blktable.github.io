...
t('only an admin can write the flag', () => {
  assert.ok(/function setFormLive\(t, on\) \{\s*\n\s*if \(!t \|\| !isAdmin\) return;/.test(SRC),
    'setFormLive is not gated on isAdmin');
});
...
t('a form that is off renders the not-found state, not a broken form', () => {
  assert.ok(/if \(tRes\.error \|\| !tRes\.data\) \{ notFound\(\); return; \}/.test(FORM),
    'a missing table row does not lead to notFound()');
});
